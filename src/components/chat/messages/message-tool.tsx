import { AnimatedShinyText } from "@/components/ui/animated-text";
...
// Your own tool invocation interface matching your parts structure
...
const MessageToolComponent: React.FC<Props> = ({ toolInvocation }) => {
  ...
  return (
    ...
    >
      <Card className="gap-0 overflow-hidden p-0 not-prose bg-card/40 border-secondary">
        <HStack className="border-b p-2">
          ...
          <AnimatePresence>
            {(toolInvocation.state === "call" ||
              toolInvocation.state === "streaming-start" ||
              toolInvocation.state === "streaming-delta") && (
              <motion.div
                initial={{
                  opacity: argsDefined ? 1 : 0,
