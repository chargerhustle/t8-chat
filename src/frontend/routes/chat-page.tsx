...
export default function Chat() {
  ...
  const handleSubmit = async (message: string, model: ModelConfig) => {
    ...
    try {
      console.log(`[CHAT] Adding user message to thread ${threadId}: ${message}`)
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full p-4">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mb-4">The thread you&apos;re looking for doesn&apos;t exist or has been deleted.</p>
  ...
  return (
    <div className="relative h-full">
      ...
      >
        ...
        >
          {messages.length > 0 ? (
            messages.map((message) => <MessageComponent key={message.messageId} message={message} />)
          ) : (
